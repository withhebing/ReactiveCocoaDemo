...
typedef NS_ENUM(NSUInteger, ICETitleViewType) {
    /// System title view
    ICETitleViewTypeDefault,
    /// Double title view
    ICETitleViewTypeDoubleTitle,
    /// Loading title view
    ICETitleViewTypeLoadingTitle
};

/// An abstract class representing a view model.
@interface ICEViewModel : NSObject
...
@property (nonatomic, strong, readonly) id<ICEViewModelServices> services;

- (instancetype)initWithParams:(NSDictionary *)params;  // temp

/// The `params` parameter in `-initWithServices:params:` method.
@property (nonatomic, copy, readonly) NSDictionary *params;

@property (nonatomic, assign) ICETitleViewType titleViewType;
