...
@implementation NSDate (ICEExtension)
...
{
    // 2014-05-01
    NSDate *nowDate = [[NSDate date] ice_dateWithYMD];

    // 2014-04-30
    NSDate *selfDate = [self ice_dateWithYMD];
...
- (BOOL) ice_isSameWeekWithAnotherDate: (NSDate *)anotherDate
{
    ...
    return (fabs([self timeIntervalSinceDate:anotherDate]) < ICE_D_WEEK);
}
...
- (BOOL) ice_isThisWeek
{
    return [self ice_isSameWeekWithAnotherDate:[NSDate date]];
}
...
{
    return [[NSDateFormatter ice_defaultDateFormatter] dateFromString:timestamp];
}
...
{
    return [[NSDateFormatter ice_defaultDateFormatter] stringFromDate:[NSDate date]];
}
...
- (NSString *)ice_formattedDateDescription
{
    ...
    if ([self ice_isThisYear])
    {
        // 今年
        if ([self ice_isToday]) {
            // 22:22
            dateFormatter = [NSDateFormatter ice_dateFormatterWithFormat:@"HH:mm"];
        }else if ([self ice_isYesterday]){
            // 昨天 22:22
            dateFormatter = [NSDateFormatter ice_dateFormatterWithFormat:@"昨天 HH:mm"];
        }else if ([self ice_isThisWeek]){
            // 星期二 22:22
            dateFormatter = [NSDateFormatter ice_dateFormatterWithFormat:[NSString stringWithFormat:@"%@ HH:mm" , [self ice_weekDay]]];
        }else{
            // 2016年08月18日 22:22
            dateFormatter = [NSDateFormatter ice_dateFormatterWithFormat:@"yyyy年MM月dd日 HH:mm"];
        }

    }else{
        // 非今年
        dateFormatter = [NSDateFormatter ice_dateFormatterWithFormat:@"yyyy年MM月dd日"];
    }
...
- (NSString *)ice_string_yyyy_MM_dd {
    return [self ice_string_yyyy_MM_dd:[NSDate date]];
}
